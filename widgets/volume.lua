...
local function worker(user_args)
  ...
  watch(
    [[sh -c 'pactl list sinks  |  grep "Name: $(pactl get-default-sink)" -A7 | grep "Name:\|Description: \|Volume: \(front-left\|mono\)\|Mute:"']],
    timeout,
    function(widget, stdout)
      ...
      if sink_muted == "no" then
        if sink_volume >= 0 and sink_volume < 10 then
          volume_icon_name = "audio-volume-very-low-symbolic"
        elseif sink_volume < 50 then
          volume_icon_name = "audio-volume-low-symbolic"
        elseif sink_volume < 90 then
  ...
  if display_notification then
    volume_widget:connect_signal("mouse::enter", function()
      show_volume_status(volume_icon_name)
    end)
